...
const syncSchema = () => {
    ...
    // Handle admin_qr column in site_settings (v27.15 LONGTEXT upgrade)
...
const settleBets = (game_name, inputNumber, arg3, arg4) => {
    let resultDate = null;
    let callback = null;
    ...
    db.query('SELECT * FROM games WHERE name = ?', [game_name.trim()], (gameErr, gameResults) => {
        ...
        db.query(query, [game_name.trim(), targetDate], (err, pendingBets) => {
            ...
            if (!pendingBets || pendingBets.length === 0) {
                console.log(`No pending bets found for ${game_name} session.`);
            ...
            const processNext = () => {
                if (processed >= pendingBets.length) {
                    console.log(`✅ ${pendingBets.length} bets for ${game_name} re-settled successfully.`);
                ...
                if (currentStatus === 'WON' && outcome === 'LOST') {
                    ...
                    console.log(`💸 DEDUCTING: ${winAmount} from UserID: ${bet.user_id}`);
                    db.query('UPDATE users SET balance = balance - ? WHERE id = ?', [winAmount, bet.user_id], (err, res) => {
                        if (err) console.error("Reversal Wallet Error:", err.message);
                        else console.log("✅ Deduction Success for ID:", bet.user_id);
                    ...
                    console.log(`💰 PAYING: ${winAmount} to UserID: ${bet.user_id}`);
                    db.query('UPDATE users SET balance = balance + ? WHERE id = ?', [winAmount, bet.user_id], (err, res) => {
                        if (err) console.error("Payout Wallet Error:", err.message);
                        else console.log("✅ Payout Success for ID:", bet.user_id);
